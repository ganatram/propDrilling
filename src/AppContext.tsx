...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case 'authenticate':
      return { ...state, loading: true }; // loading = true -- state change
    case 'authenticated':
      return { ...state, loading: false, user: action.user }; // { id: '1', name: 'Bob' }
    case 'authorize':
      return { ...state, loading: true }; // loading = true
    case 'authorized':
      return {
        ...state,
        loading: false, // loading = false
        permissions: action.permissions, // ['admin']
      };
    default:
      return state;
  }
